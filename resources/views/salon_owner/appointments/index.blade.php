<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-glow-primary leading-tight">{{ __('Appointments') }}</h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="GET" class="mb-6 flex flex-wrap gap-2">
                ...
                <input type="date" name="date" value="{{ request('date') }}" class="rounded-md border-gray-300 shadow-sm focus:border-glow-accent focus:ring-glow-accent text-sm">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right text-sm space-x-1">
                                        @if(in_array($appt->status, ['confirmed', 'pending']))
                                            <form method="POST" action="{{ route('salon-owner.appointments.cancel', $appt) }}" class="inline">@csrf @method('PATCH')
                                                <button onclick="return confirm('Cancel?')" class="text-red-600 hover:text-red-800 font-medium">Cancel</button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
